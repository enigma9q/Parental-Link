...
public class AttentionActivity extends Activity{
    boolean blocking=false; boolean closing=false; android.content.BroadcastReceiver closeReceiver; Handler handler=new Handler(Looper.getMainLooper());
    Runnable refocusRunnable=new Runnable(){public void run(){if(blocking&&!closing&&activeChildLock()){bringLockScreenToFront();}}};
    public void onCreate(Bundle b){
        super.onCreate(b);
        applyIntent(getIntent());
        buildScreen();
        closeReceiver=new android.content.BroadcastReceiver(){public void onReceive(android.content.Context c,android.content.Intent i){closing=true;finish();}};
    ...
    protected void onNewIntent(Intent intent){super.onNewIntent(intent);setIntent(intent);applyIntent(intent);buildScreen();}
    void applyIntent(Intent intent){blocking=intent!=null&&intent.getBooleanExtra("blocking",false);}
    void buildScreen(){
        ...
        keepFrontVisuals();
        LinearLayout root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setGravity(Gravity.CENTER);root.setPadding(40,40,40,40);root.setBackgroundColor(Color.rgb(245,248,255));
    ...
    void keepFrontVisuals(){
        ...
        if(Build.VERSION.SDK_INT>=21){getWindow().setStatusBarColor(Color.rgb(245,248,255));getWindow().setNavigationBarColor(Color.rgb(245,248,255));}
    }
    boolean activeChildLock(){return getSharedPreferences("rules",0).getLong("lock_until",0)>System.currentTimeMillis();}
    void bringLockScreenToFront(){try{Intent i=new Intent(this,AttentionActivity.class);i.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK|Intent.FLAG_ACTIVITY_CLEAR_TOP|Intent.FLAG_ACTIVITY_SINGLE_TOP|Intent.FLAG_ACTIVITY_NO_ANIMATION);i.putExtra("title",getIntent().getStringExtra("title"));i.putExtra("text",getIntent().getStringExtra("text"));i.putExtra("reason",getIntent().getStringExtra("reason"));i.putExtra("blocking",true);startActivity(i);}catch(Exception ignored){}}
    void scheduleRefocus(long delay){handler.removeCallbacks(refocusRunnable);if(blocking&&!closing&&activeChildLock())handler.postDelayed(refocusRunnable,delay);}
    protected void onResume(){super.onResume();handler.removeCallbacks(refocusRunnable);if(blocking&&!activeChildLock()){closing=true;finish();return;}keepFrontVisuals();}
    protected void onPause(){super.onPause();scheduleRefocus(180);}
    protected void onStop(){super.onStop();scheduleRefocus(350);}
    public void onWindowFocusChanged(boolean hasFocus){super.onWindowFocusChanged(hasFocus);if(hasFocus)keepFrontVisuals();else scheduleRefocus(250);}
    public void onBackPressed(){if(!blocking)super.onBackPressed();}
    protected void onDestroy(){handler.removeCallbacksAndMessages(null);try{if(closeReceiver!=null)unregisterReceiver(closeReceiver);}catch(Exception ignored){}super.onDestroy();}
}
